      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14">
          {stats.map((stat, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl p-5 text-center border border-border shadow-sm"
            >
              <div className="font-serif text-3xl md:text-4xl font-bold text-primary leading-none">
                {stat.value}
                {stat.sublabel && (
                  <span className="text-xl md:text-2xl"> {stat.sublabel}</span>
                )}
              </div>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-center md:items-start text-center md:text-left">
            ...
            <div className="flex-1">
              ...
              <p className="text-muted-foreground leading-relaxed text-sm md:text-base max-w-2xl">
                Con más de una década de experiencia en estética integral, combina tecnología avanzada
                con un profundo conocimiento del cuerpo femenino. Su enfoque es simple: resultados reales,
                atención personalizada y que cada paciente salga sintiéndose mejor que cuando entró.
              </p>
            </div>
          </div>
        </div>

        {/* Grid de puntos de confianza */}
